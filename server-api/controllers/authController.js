    ...
    if (dbUser) {
      ...
      if (userEncryptedDetails.encryptedPassword === dbUser.password) {
        ...
        res
          ...
          .send(badRequestResponse("Unauthorized", HTTPCodes.UNAUTHORIZED));
      }
    } else {
      res
        .status(HTTPCodes.NOT_FOUND)
        .send(badRequestResponse("Email does not exist", HTTPCodes.NOT_FOUND));
    }
  } catch (e) {}
}
  ...
  try {
    ...
    _.forIn(requiredBody, function (value, key) {
      if (!req.body[key]) {
        ...
        req.body[key] = value ? value.toString().trim() : "";
      }
    });
    ...
    if (e.toString().includes("Duplicate entry")) {
      res
        ...
        .send(badRequestResponse("Email/dni already exists"));
    } else {
